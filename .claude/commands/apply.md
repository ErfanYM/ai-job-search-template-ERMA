...
**Template:** the single-page resume template (`cv/main_example.tex`, compiled with `pdflatex`) and the `cover.cls` cover letter (`xelatex`). `/add-template` is not expected to be used here; if a `05`/`06` file ever opens with an `ACTIVE-TEMPLATE` managed block, honor its declared source extension and compile command instead. Otherwise: `.tex`, `pdflatex` (twice) for the resume, `xelatex` for the cover letter.
...
### CV (`cv/main_<company>_<role>.tex`) - single-page resume
- Match the candidate's CV language from their profile. Never switch language per posting.
- Follow the **single-page template** from `05-cv-templates.md`. This is NOT moderncv. Build from `cv/main_example.tex` by cutting and reframing.
- Hard rules from `05-cv-templates.md`: **exactly 1 page, fully used**; no summary/objective section; section order Skills -> Projects -> Experience -> Education (Projects before Experience); Experience entries lead with job title (company second, italic); bold 1-2 keywords per bullet; prune Technical Skills to the posting; no em-dashes; `\resumeSubheading` date args use a single ASCII hyphen.
- Pick the honest job-title variant per posting from the Title framing notes in `01-candidate-profile.md`.
- Reframe skills and project/experience bullets to the job requirements; keep every metric exact.
- **Grounding Audit:** Before writing to disk, audit all tailored bullets against the union of four sources: `01-candidate-profile.md` + `cv/main_example.tex` + `bird_view.md` + `CLAUDE.md`'s Candidate Profile section. A claim is grounded if ANY of them supports it. All dates, roles, and metrics must match exactly (zero drift or fabrication). The degree is complete (May 2026) - never "pursuing" or "expected".
...
- any narrative fact-source file created by `/setup` (fullest account of the candidate's experience)
...
```bash
cd cv && pdflatex -interaction=nonstopmode main_<company>_<role>.tex && pdflatex -interaction=nonstopmode main_<company>_<role>.tex
cd ../cover_letters && xelatex -interaction=nonstopmode cover_<company>_<role>.tex
```

- **The resume** uses **pdflatex**, run twice (references settle). The template sets `\pdfgentounicode=1` and `\input{glyphtounicode}` for clean ATS text extraction. Do not use lualatex/xelatex for the resume.
- **Cover letter** uses **xelatex** — `cover.cls` requires fontspec (Lato/Raleway fonts in `cover_letters/OpenFonts/`).
- **Custom template active:** run its declared `<CV_COMPILE>`/`<COVER_COMPILE>` command instead.
...
**Resume (`cv/main_<company>_<role>.pdf`):**
- [ ] **Exactly 1 page** (check the pdflatex log line "Output written on ... (1 page")
- [ ] No section heading or entry title orphaned at the very bottom with its content on a (non-existent) page 2
- [ ] Last line of text sits reasonably close to the bottom margin - **no large empty gap** at the end of the page
- [ ] Fill check (no need to open the PDF):
  ```bash
  ...
  ```
  `pages` must be 1; `lowest_y` around 30-45 = well filled; above ~70 = add content.
...
- **Resume spills to page 2:** cut with **relevance-weighted cutting** (see `05-cv-templates.md`). Score each candidate line by (a) relevance to THIS posting's keywords and responsibilities, (b) uniqueness, (c) whether the cover letter depends on it. Cut the lowest-total-score line first, regardless of section. First cuts: a skills item duplicated by a bullet, a low-relevance older bullet, dropping a 4th project to 3, a role's weakest 3rd bullet. Last resort: tighten `\resumeItemListStart` itemsep from `-3pt` to `-4pt`, or merge two short bullets. Never shrink font or page geometry.
- **Resume ends high on the page (thin):** restore the highest-relevance line previously cut, add a 3rd bullet to a 2-bullet project, or add a skills item.
...
- [ ] **Email and phone survive as literal text.** The template prints the phone number and email as plain text in the header (the email is also a `mailto:` link, but the visible text is what matters). Both must appear verbatim in the extraction. The LinkedIn and `Github` link texts are fine as-is - those are not screening fields.
- [ ] **Reading order matches the visual order** - section headings in page sequence (Technical Skills, Projects, Experience, Education), no interleaving. The template is single-column and safe.
- [ ] **Dates recognizable and ASCII-hyphenated** - each Experience entry and the Education entry shows its dates with a plain hyphen (`Jan 2025 - May 2025`), not an en-dash. An en-dash in a `\resumeSubheading` date arg is a silent Workday/Greenhouse import failure - fix it in the source if the extraction shows `2025<U+2013>2026`.
...
## Step 7: The outward pipeline (rename -> approval -> Drive -> Notion -> commit)

Steps 7a and 7b run **every time, unconditionally**, immediately after Step 6. Steps 7c-7f run **only after the user explicitly approves** in 7b.

### 7a. Rename the PDFs (always - never gated, never skipped)

Ask for the company name and job title if they are not already unambiguous, then `cp` (never `mv` - keep the `cv/` and `cover_letters/` originals for future builds):

```bash
cp cv/main_<company>_<role>.pdf <Company>_<YourName>_<JobTitle>.pdf
```

**Always** make the renamed cover-letter copy too, every application (it is always uploaded to Drive in 7c, independent of whether the portal wants it as a separate submission file):
```bash
cp cover_letters/cover_<company>_<role>.pdf <Company>_<YourName>_<JobTitle>_CoverLetter.pdf
```

Filename form: `PepsiCo_YourName_JuniorDataScientist.pdf` - CamelCase, no spaces or separators inside the name. The renamed copies live at the repo root.

Renaming happens here regardless of whether the user has approved submission. "Submit" later means do 7c-7f, NOT redo 7a.

### 7b. Ask whether it is good to submit

Ask plainly: **"Is this resume + cover letter good to submit?"**

This gate runs **every time, for every job, even if the user said earlier in the session to skip approval** - a mid-session "skip the approval" is not durable unless it is written into `CLAUDE.md`. Only a yes here proceeds to 7c. If no, stop; the user will iterate.

### 7c. Upload to Google Drive (separate folders for resume vs cover letter)

```bash
python3 drive_sync.py "<Company>_<YourName>_<JobTitle>.pdf" <RESUME_FOLDER_ID>
```
Set `<RESUME_FOLDER_ID>` to your own Google Drive resume folder id (see `SETUP.md`).

**Always** upload the cover-letter PDF to the cover-letter folder too, every application (not only when the portal wants it as a separate file):
```bash
python3 drive_sync.py "<Company>_<YourName>_<JobTitle>_CoverLetter.pdf" <COVER_LETTER_FOLDER_ID>
```
Set `<COVER_LETTER_FOLDER_ID>` to your own Google Drive cover-letter folder id.

Keep both links. The resume link is the one passed to `notion_sync.py` in 7d.

### 7d. Insert the Notion tracker row

```bash
python3 notion_sync.py "<Company>" "<Job Title>" "<job posting URL>" "<drive link from 7c>"
```
- Job URL: use the URL the user pasted with the posting. Do not re-ask. Only ask if no URL was ever given; if they say skip, pass `""`.
- Stage defaults to "To apply". Pass `"Applied"` as a 5th arg only if the user explicitly says they already applied. Other stages: Interview, No Answer, Offer, Rejected.

### 7d.5. Find a Chicken (hiring contact search brief)

Per `.claude/skills/job-application-assistant/10-hiring-contacts.md`: produce the hiring-manager
and recruiter LinkedIn search brief now, from the posting text and company name already held in
context. No WebSearch/WebFetch by default - this is a reasoning pass, not a research pass. Print
the report block to chat; it is never written to the tracker, Drive, or git.

### 7e. Update the tracker row status

The `job_search_tracker.csv` row from Step 6b is still `drafted`. Leave it as `drafted` here - the user runs `/outcome <company>` after they actually submit on the portal, which moves it to `applied`. (The Notion row is the "to apply" signal; the CSV stays honest about what was actually sent.)

### 7f. Commit to git

Commit on the **current branch** with a message in this repo's style: `<Company>-<Role>` (e.g. `TD-AssociateSoftwareEngineer`, `GreenShield-BusinessDataAnalyst`).

- **Before committing:** `ls *.pdf` at the repo root. `git rm` any tailored `<Company>_<YourName>_<JobTitle>.pdf` from a **previous** job posting (recoverable from that job's own commit). Only the current job's renamed PDF(s) stay tracked.
- The commit should touch only: the current job's renamed PDF(s), `cv/main_<company>_<role>.tex`, `cover_letters/cover_<company>_<role>.tex`, `job_search_tracker.csv`, `documents/applications/<company>_<role>/`, and any removed prior-job PDFs. Do not sweep in unrelated working-tree changes.
- End the commit message with the two trailer lines this repo uses (Co-Authored-By: Claude ... and Claude-Session: ...).

Report the Drive link, the Notion link, and the commit hash.
