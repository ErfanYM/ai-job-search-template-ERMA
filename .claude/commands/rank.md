...
Dispatch parallel `general-purpose` agents via the **Agent tool**, ~5 jobs per agent (a single agent is fine for ≤5 jobs). **Pass `model: "haiku"` (or the cheapest available) to these scoring agents** - ranking is deliberately shallow triage and does not need Opus. Never draft a CV or cover letter during ranking. Token-efficiency rules, consistent with `/apply`:
